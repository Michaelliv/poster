export default function Concert() {
  return (
    <div className="relative w-[1200px] overflow-hidden p-10" style={{ background: "#0b0a0c", fontFamily: "'Inter', sans-serif" }}>
      {/* noise texture */}
      ...
      <div className="relative h-full">
        <div className="absolute top-0 left-0 right-0 flex items-baseline justify-between">
          <span className="text-[14px] font-mono uppercase tracking-[0.3em] text-white/40">TRACK LIST 01 → 14</span>
          <span className="text-[14px] font-mono uppercase tracking-[0.3em] text-white/40">SIDE A</span>
        </div>

        {/* FAT TITLE */}
        <div className="pt-10 text-white">
          <div className="text-[15px] font-bold uppercase tracking-[0.5em] text-[#f97316]">Live at Tanzhaus · Berlin</div>
        ...
        <div className="mt-6 flex items-center justify-between border-y border-white/15 py-4">
          ...
          <div className="text-[14px] font-mono uppercase tracking-wider text-white/50">Doors 19:00 · Sold out</div>
        </div>
          ...
          <div className="space-y-3">
            {[
              ...
              <div key={n} className="flex items-baseline gap-3 border-b border-white/10 pb-2">
                <span className="w-8 font-mono text-sm text-white/40">{n}</span>
                <span className="flex-1 text-lg font-bold uppercase tracking-tight">{name}</span>
                <span className="text-[14px] uppercase tracking-[0.2em] text-[#f97316]">{role}</span>
              </div>
            ))}
          </div>
          <div className="space-y-3">
            <div className="rounded-2xl border border-white/15 bg-white/[0.03] p-5">
              <div className="text-[14px] font-bold uppercase tracking-[0.3em] text-white/50">Venue</div>
            ...
            <div className="rounded-2xl border border-white/15 bg-white/[0.03] p-5">
              <div className="text-[14px] font-bold uppercase tracking-[0.3em] text-white/50">Tickets</div>
            ...
            <div className="rounded-2xl border border-[#f97316]/40 bg-[#f97316]/10 p-5">
              <div className="text-[14px] font-bold uppercase tracking-[0.3em] text-[#f97316]">All ages · vinyl · cash only</div>
        ...
        <div className="mt-6 flex flex-wrap gap-2">
          {["ambient", "drone", "post-techno", "IDM", "modular", "generative", "field-recording"].map((t) => (
            <span key={t} className="rounded-full border border-white/20 px-3 py-1 text-[14px] font-mono uppercase tracking-wider text-white/70">{t}</span>
          ))}
        </div>
      </div>
    </div>
